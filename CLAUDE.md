...
- **Single precision** (`hc.solve_nested(..., single_precision=True)`): runs each level's H2 matvec + PCG in `float`, ~halving peak RAM for memory-bound nodes at the largest grids (e.g. Ns=16384, N≈2.7e8). `solve_contact` is templated (`solve_contact_impl<Real>`, float/double); `H2Operator::matvec_single`/`build_single_caches` hold float cache copies; `FourierPreconditioner::apply_single`. Float's arithmetic floor is ~1e-6, so the finest tol is clamped to 2e-6 in this mode (solution still matches double to rel-L2 ~2e-5, ΔArea ~4e-6). Default `False` — nothing changes for existing calls.
