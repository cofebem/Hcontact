...
is \code{FourierPreconditioner::apply} (\code{fourier\_precond.cpp:57--83}); the
2-D transform is done as $1$-D Eigen FFTs over rows then columns
(\code{:21--55}), and the final contact-mean removal (\code{:72--81}) is the
discrete analogue of the $\mathbf q=0$ zeroing on the \emph{reduced} space.
...
produces the \emph{same} iterates as the original Polonsky--Keer PCG, to the last
bit.
\end{proposition}
\begin{proof}
With no preconditioner the code sets $z_i=g_i$ on $\mathcal A$ and $0$ elsewhere
(\code{contact\_solver.cpp:67--68}). Then $G=\langle g,g\rangle_{\mathcal A}$ and
...
$t=g+\beta t$ is the original direction. No other line is touched. Hence every
arithmetic operation matches the original code path. (Verified numerically in
\code{tests/test\_precond.cpp}, and as the \code{cpp}$=$\code{none} columns of
Table~\ref{tab:precond-it}: the C++ baseline and the Python prototype agree
exactly.)
\end{proof}
